...
int main(int argc, char* argv[])
{
	...
	int				numberBytes;
	clock_t			goal;
	clock_t			wait=10;		// 10 ms delay used to "pace" the calls to get a data record
	...
	SET_SYSTEM_PARAMETER(MEASUREMENT_RATE,		75.0);
	...
	SET_TRANSMITTER_PARAMETER(transmitterID, XYZ_REFERENCE_FRAME, true);
	...
	//
	errorCode = SaveSystemConfiguration("c:\\newfile.ini");
	if(errorCode!=BIRD_ERROR_SUCCESS) errorHandler(errorCode);
